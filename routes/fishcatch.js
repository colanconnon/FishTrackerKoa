...
router.get('/api/fishcatch/:id', function * () {
  ...
  var sql = `
  SELECT fishcatch.id, latitude, longitude, details, temperature, lake_id, fishcatch.user_id, users.username, lake.lakeName
  FROM fishcatch
...
router.post('/api/fishcatch/insert', function * () {
  ...
  var sql = `
    INSERT INTO public.fishcatch(
     latitude, longitude, details, temperature, lake_id, user_id)
    VALUES ($1, $2, $3, $4, $5, $6) RETURNING Id;
  `;
  var result = yield this.pg.db.client.query_(sql, [fishCatch.latitude, fishCatch.longitude, fishCatch.details, 
                                                   fishCatch.temperature, fishCatch.lake_id,this.state.user.id]);
  console.log(result);
